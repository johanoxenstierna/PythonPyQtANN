...
class Controller(QMainWindow):
    ...
    def initialize_and_draw_lineEdits_and_buttons(self):
        ...
        # weights (shouuld be done with for each obviously)
        # top neuron
        for j in range(0, 3):
            c = m_net.get_coordinates(m_net.get_neuron(0, 0))
            my_line_edit_w = QPushButton(self)
            my_line_edit_w.setGeometry(c[0] + 100, c[1] - 20 + (22 * j), 60, 20)
            self.synapse_buttonsL0L1[0, j] = my_line_edit_w

        # mid neuron 1
        for j in range(0, 3):
            c = m_net.get_coordinates(m_net.get_neuron(0, 1))
            my_line_edit_w = QPushButton(self)
            my_line_edit_w.setGeometry(c[0] + 100, c[1] - 30 + (22 * j), 60, 20)
            self.synapse_buttonsL0L1[1, j] = my_line_edit_w

        # bottom neuron
        for j in range(0, 3):
            c = m_net.get_coordinates(m_net.get_neuron(0, 2))
            my_line_edit_w = QPushButton(self)
            my_line_edit_w.setGeometry(c[0] + 100, c[1] - 40 + (22 * j), 60, 20)
            self.synapse_buttonsL0L1[2, j] = my_line_edit_w
        ...
        for j in range(0, 3):
            ...
            my_line_edit_bw.setGeometry(cb[0] + 100, cb[1] - 40 + (22 * j), 60, 20)
            self.synapse_buttonsL0L1[3, j] = my_line_edit_bw
    ...
    def update_lineEdits_and_buttons(self): #xx4 major changes
        ...
        for synapse in synapses:
            ...
            QPushButton.setText(self.synapse_buttonsL0L1[synapse[0], synapse[1]], str(round(weight, 4)))
        ...
        for i in range(0, 3):
            ...
            QPushButton.setText(self.synapse_buttonsL0L1[3, i], str(round(bias_w, 4)))
        ...
        # update second layer
        # set inputs and outputs
        for i in range(0, 3): #change to 5?
        ...
        #bias
        bias_o = m_net.get_output(m_net.get_neuron(1, 3))
        QLineEdit.setText(self.l1outputs[3][1], str(bias_o))
        bias_w = getattr(synapses[3, 0], 'weight')
        QPushButton.setText(self.synapse_buttonsL1L2[3, 0], str(round(bias_w, 4)))

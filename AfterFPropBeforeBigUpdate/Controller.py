...
class Controller(QMainWindow):
    ...
    def forward_propL1(self):
        ...
        for i in range(0, 4):
            ...
            this_neuron = m_net.get_neuron(1, i)
            m_net.set_input(this_neuron, str(sum))

            n_output = self.transfer_function(sum)
            m_net.set_output(this_neuron, n_output)
    ...
    def forward_propL2(self):
        ...
        for i in range(0, 4):
            ...
            prev_neuron = m_net.get_neuron(1, i)
            prev_neuron_output = m_net.get_output(prev_neuron)
            weight = m_net.get_weight(1, i, 0)

            sum += prev_neuron_output * weight

            this_neuron = m_net.get_neuron(2, 0)
            m_net.set_input(this_neuron, str(sum))
